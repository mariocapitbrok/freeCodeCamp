...
function getRating(watchList) {
  ...
  const christopherNolanRatings = watchList
    .filter(m => m.Director === 'Christopher Nolan')
    .map(m => m.imdbRating)
  const sum = christopherNolanRatings.reduce(
    (sum, rating) => sum + Number(rating),
    0
  )
  let averageRating = sum / christopherNolanRatings.length
